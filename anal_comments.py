...
# 计算时间趋势变化
def TimeAnal(cleaned_df, cat, start_time='2025-10-31 12:00:00', end_time='2025-11-01 23:59:59', freq='2H'):
    """
    计算随时间的一些数据，cleaned_df、cat必要，其余可选
    cleaned_df: 仅写清理过的dataframe名，列在函数中写有
    cat: '点赞' or '评论' 随时间分布
    start_time: default '2025-10-31 12:00:00'
    end_time: default '2025-11-01 23:59:59'
    freq: default '2H'
    """
...
def UseTimeAnal(cleaned_df, cat, folder, start_time='2025-10-31 12:00:00', end_time='2025-11-01 23:59:59', freq='2H'):
    # TimeAnal(cleaned_df, cat, start_time='2025-10-31 12:00:00', end_time='2025-11-01 23:59:59', freq='2H')
    """
    嵌套TimeAnal函数，用来进行点赞数、评论数的分析、可视化展示、保存
    保存路径为 r'D:\pycharmdata\pythonProject\self\\test\{}.png'.format(cat)
    参数和TimeAnal设置的一样
    cleaned_df: 清洗、去重后的dataframe
    cat： ‘点赞’ or ‘评论’
    folder: 文件夹名，命名成分析的对象名，如终末地
    start_time： 可选，default'2025-10-31 12:00:00'
    end_time: 可选，default'2025-11-01 23:59:59'
    freq: 步长，可选，default'2H'
    """
    t_stats = TimeAnal(cleaned_df, cat, start_time=start_time, end_time=end_time, freq=freq)
    ...
    plt.tight_layout()
    plt.savefig(r'D:\pycharmdata\pythonProject\self\test\{}\{}.png'.format(folder, cat))
    plt.show()
...
like_anal = UseTimeAnal(cleaned_df, '点赞', '终末地')
cmt_anal = UseTimeAnal(cleaned_df, '评论', '终末地', end_time='2025-11-04 23:59:59')

# 2 - 终末地玩法
df = OpenCsv(r'D:\pycharmdata\pythonProject\self\test\zmdwanfa.csv')
top_accounts = GetTopAccounts(df)
cleaned_df = GetCleanedDf(df)
wcloud, keywords_df = BuildWordCloud(cleaned_df)
SaveSheets('终末地玩法')
UseWordCloud(wcloud, '终末地玩法')
InfosAnal(cleaned_df, '等级', '终末地玩法')
InfosAnal(cleaned_df, '性别', '终末地玩法')
like_anal = UseTimeAnal(cleaned_df, '点赞', '终末地玩法', start_time='2025-11-12 10:00:00', end_time='2025-11-14 23:59:59')
cmt_anal = UseTimeAnal(cleaned_df, '评论', '终末地玩法', start_time='2025-11-12 10:00:00', end_time='2025-11-16 23:59:59')